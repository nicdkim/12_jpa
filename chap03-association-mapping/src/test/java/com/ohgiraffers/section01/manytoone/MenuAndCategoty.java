...
/*
* @JoinColum 은 다대일 연관 관계에서 사용되는 어노테이션이다.
...
* - insertable : 새로운 엔티티가 저장될 때, 이 참조 컬럼이 sql insert 문에 포함될지 지정한다.
* - updateable : 엔티티가 업데이트 될 떄, 이 참조 컬럼이 update 문에 포함될지 지정한다.
* - table : 참조하는 테이블의 이름을 지정한다.
* - foreingnkey : 참조하는 테이블에 생성될 외래 키에 대한 추가 정보를 지정한다.
*
...
//    @ManyToOne(cascade = CascadeType.MERGE)
    ...
    public MenuAndCategoty() {
    }

    public MenuAndCategoty(int menuCode, String menuName, int menuPrice, Category category, String orderableStatus) {
